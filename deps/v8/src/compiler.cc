...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
  const auto aot_compile = info->isolate()->is_doing_aot_compilation();
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
#ifdef ENABLE_JSAOT_
    if (aot_compile)
      // Currently AOT doesn't support wasm generated code.
      return false;
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
  // If now is in aot compile mode and not bootstrapper, skip to compile
  // asm function as currently AOT doesnot support asm generate native code.
  if (aot_compile && !info->isolate()->bootstrapper()->IsActive() &&
       (info->scope()->IsAsmModule() || info->scope()->IsAsmFunction())) {
    return false;
  }
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
