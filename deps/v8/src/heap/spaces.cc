...
#ifdef ENABLE_JSAOT_
// <YUNOS> changed: begin
  if (kRequiresCodeRange && isolate_->jso_range_enabled()) {
    code_range_ = new base::VirtualMemory();
    JsoRange::TakeControlOfCodeRange(code_range_);
    requested = code_range_->size();
  } else {
    code_range_ = new base::VirtualMemory(
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
  if (main_isolate_->jso_range_enabled())
    jso_range_.SetUp();
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
// <YUNOS> changed: begin
    ...
    const char* name, Space* owner, size_t* allocated) {
// <YUNOS> changed: end
#else
    Executability executable, void* hint, base::VirtualMemory* controller,
    const char* name) {
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
  if (jso_range_.IsValid() && jso_range_.ShouldAllocateInJsoRange(owner)) {
    auto base = jso_range_.AllocateRawMemory(reserve_size, commit_size,
                                             name, executable, allocated);
    if (base) {
      if (executable == EXECUTABLE)
        size_executable_.Increment(*allocated);
      size_.Increment(*allocated);
    }
    return base;
  }
#endif  // ENABLE_JSAOT_
...
bool MemoryChunk::CommitArea(size_t requested) {
  ...
  if (commit_size > committed_size) {
    ...
    // <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
    } else if (heap_->memory_allocator()->jso_range()->Contains(address())) {
      Executability executable =
          IsFlagSet(IS_EXECUTABLE) ? EXECUTABLE : NOT_EXECUTABLE;
      if (!heap_->memory_allocator()->jso_range()->CommitRawMemory(start, length,
                                                                   executable))
        return false;
#endif  // ENABLE_JSAOT_
    // <YUNOS> changed: end
...
// <YUNOS> changed: begin
      ...
      if (!heap_->memory_allocator()->jso_range()->UncommitRawMemory(start, length))
        return false;
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
    } else {
...
#ifdef ENABLE_JSAOT_
// <YUNOS> changed: begin
                                   address_hint, &reservation, space_name, owner,
                                   &chunk_size);
// <YUNOS> changed: end
#else
...
#ifdef ENABLE_JSAOT_
// <YUNOS> changed: begin
  if (jso_range_.Contains(chunk->address())) {
    jso_range_.FreeRawMemory(chunk->address(), chunk->size());
  } else if (chunk->IsFlagSet(MemoryChunk::POOLED)) {
// <YUNOS> changed: end
#else
...
// <YUNOS> changed: begin
  if (alloc_mode == kPooled &&
      owner->heap_safe()->gc_state() != Heap::AOT_MARK_MIGRATE) {
// <YUNOS> changed: end
#else
  if (alloc_mode == kPooled) {
#endif  // ENABLE_JSAOT_
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
void MemoryAllocator::LinkCompiledPage(MemoryChunk* chunk, Space* owner) {
  const auto base = reinterpret_cast<Address>(chunk);
  const auto size = chunk->size();
  size_.Increment(size);
  UpdateAllocatedSpaceLimits(base, base + size);
  chunk->heap_ = main_isolate_->heap();
  chunk->set_owner(owner);
  chunk->skip_list_ = nullptr;
  chunk->progress_bar_ = 0;
  chunk->concurrent_sweeping_state().SetValue(MemoryChunk::kSweepingDone);
  chunk->mutex_ = new base::RecursiveMutex();
  chunk->available_in_free_list_ = 0;
  chunk->wasted_memory_ = 0;
  chunk->local_tracker_ = nullptr;
  jso_range()->AddLoadedBlock(base, size);
}
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
void PagedSpace::LinkCompiledPage(Page* page) {
  heap()->memory_allocator()->LinkCompiledPage(page, this);
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
    if (page->IsFlagSet(Page::AOT_COMPILED))
      continue;
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
  if (heap()->memory_allocator()->jso_range()->Contains(address())) {
    return 0;
  }
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
void LargeObjectSpace::LinkCompiledPage(LargePage* page) {
  heap()->memory_allocator()->LinkCompiledPage(page, this);
  size_ += static_cast<int>(page->size());
  AccountCommitted(page->size());
  objects_size_ += page->GetObject()->Size();
  page_count_++;
  page->set_next_page(first_page_);
  first_page_ = page;
  InsertChunkMapEntries(page);
