...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
std::map<std::string, Local<UnboundScript>> Shell::aot_scripts_;
#endif  // ENABLE_JSAOT
// <YUNOS> changed: end
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
bool Shell::ExecuteScript(Isolate* isolate, Local<v8::Script> script) {
...
// <YUNOS> changed: begin
    ...
    auto result = aot_scripts_.find(absolute_path);
    if (result != aot_scripts_.end()) {
      // printf("AOT script %s hit\n", *file);
      auto script = result->second->BindToCurrentContext();
      if (!ExecuteScript(args.GetIsolate(), script)) {
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
    std::string s_file_name = std::string(arg);
    std::string absolute_path = NormalizePath(s_file_name, GetWorkingDirectory());
    auto result = Shell::aot_scripts_.find(absolute_path);
    if (result != Shell::aot_scripts_.end()) {
      // printf("AOT script %s hit\n", arg);
      auto script = result->second->BindToCurrentContext();
      if (!Shell::ExecuteScript(isolate, script)) {
        exception_was_thrown = true;
        break;
      }
      continue;
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
    } else if (strncmp(argv[i], "--aot_in=", 9) == 0) {
      // Make sure the context layout of a function recompiled by full-codegen
      // is same as the layout by AOT compilation, which assumes --no-lazy.
      SetFlagsFromString("--no-lazy_inner_functions");
      options.jso_file = argv[i] + 9;
      argv[i] = NULL;
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
      if (last_run && options.jso_file) {
        TryCatch try_catch(isolate);
        try_catch.SetVerbose(true);
        JsoLoader loader(isolate);
        const auto fd = open(options.jso_file, 0);
        const auto num_loaded_scripts = fd >= 0 ? loader.Load(fd) : 0;
        if (num_loaded_scripts < 0) {
          DCHECK(try_catch.HasCaught());
          ReportException(isolate, &try_catch);
          return -1;
        }
        for (int i = 0; i < num_loaded_scripts; i++) {
          auto script = loader.GetUnboundScript(i).ToLocalChecked();
          String::Utf8Value name(script->GetScriptName());
          aot_scripts_[std::string(*name)] = script;
        }
      }
  #endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
  Isolate* isolate = Isolate::New(create_params, options.jso_file != nullptr);
#else
  Isolate* isolate = Isolate::New(create_params);
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
