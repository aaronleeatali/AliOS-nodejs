...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
JsoGenerator::JsoGenerator(Isolate* isolate) {
  generator_ = new i::JsoGenerator(isolate);
}
...
void JsoGenerator::Output(FILE* out) {
  i::OFStream os(out);
  generator_->Output(os);
}

void JsoGenerator::Print(FILE* out) {
  i::OFStream os(out);
  generator_->Print(os);
}

JsoLoader::JsoLoader(Isolate* isolate) {
  loader_ = new i::JsoLoader(isolate);
}
...
int JsoLoader::Load(int fd, bool invertedReplaceFlag) {
  i::ApiObjectAccessScope aoa(loader_->isolate());
  return loader_->Load(fd, invertedReplaceFlag);
}

MaybeLocal<UnboundScript> JsoLoader::GetUnboundScript(unsigned i) const {
  i::ApiObjectAccessScope aoa(loader_->isolate());
  return loader_->GetUnboundScript(i);
}
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
// <YUNOS> changed: begin
#ifdef ENABLE_JSAOT_
bool v8::V8::InitializeJsoRange(uintptr_t raw_addr, size_t size) {
  return i::JsoRange::InitializeJsoRange(raw_addr, size);
}
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
Isolate* Isolate::New(const Isolate::CreateParams& params,
                      bool enable_jso_range) {
/*YUNOS BEGIN*/
#ifdef ENABLE_JSAOT_
  i::Isolate* isolate = new i::Isolate(false, nullptr, enable_jso_range);
#else
