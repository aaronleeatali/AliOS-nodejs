...
/* YUNOS BEGIN */
#ifdef ENABLE_JSAOT_
class V8_EXPORT JsoGenerator {
 public:
  JsoGenerator(Isolate* isolate);
  ~JsoGenerator();
 ...
 private:
  internal::JsoGenerator *generator_;
};

class V8_EXPORT JsoLoader {
 public:
  JsoLoader(Isolate* isolate);
  ~JsoLoader();

  // Load jso file
  // fd: file handle of jso file
  // invertedReplaceFlag:
  //  true: to replace duplicate internalized string
  //        in jso file with its corresponding string in stringtable.
  //  false: to replace duplicate internalized string in stringtable
  //        with its corresponding string in jso file
  int Load(int fd, bool invertedReplaceFlag = false);

  MaybeLocal<UnboundScript> GetUnboundScript(unsigned i) const;

 private:
  internal::JsoLoader *loader_;
};
#endif  // ENABLE_JSAOT_
/* YUNOS END */
...
class V8_EXPORT Isolate {
 public:
   ...
   */
  static Isolate* New(const CreateParams& params, bool enable_jso_range = false);
...
#ifdef ENABLE_JSAOT_
  /**
   * Initialize the global JsoRange object for compiling or loading jso file.
   * This must be called before any mmap calls to make sure the created address
